...
try:
    ...
    print("The e3nn is not installed, mapper cannot handle irreps related maps, be careful!")
...
class BondMapper(TypeMapper):
    def __init__(
        ...
        ZZ_to_index = np.full(
                (len(self._Z_to_index), len(self._Z_to_index)), fill_value=-1, dtype=np.int64
            )
        ...
        self._index_to_ZZ = np.zeros(
                (len(self.bond_to_type),2), dtype=np.int64
            )
        self._reduced_index_to_ZZ = np.zeros(
                (len(self.reduced_bond_to_type),2), dtype=np.int64
            )
        
        for abond, aidx in self.bond_to_type.items():
            asym, bsym = abond.split("-")
            self._index_to_ZZ[aidx] = np.asarray([ase.data.atomic_numbers[asym], ase.data.atomic_numbers[bsym]], dtype=np.int64)

        for abond, aidx in self.reduced_bond_to_type.items():
            asym, bsym = abond.split("-")
            self._reduced_index_to_ZZ[aidx] = np.asarray([ase.data.atomic_numbers[asym], ase.data.atomic_numbers[bsym]], dtype=np.int64)
...
class OrbitalMapper(BondMapper):
    def __init__(
        ...
        self.atom_norb = np.zeros(len(self.type_names), dtype=np.int64)
        for ib in self.basis.keys():
        ...
        self.mask_to_basis = np.zeros((len(self.type_names), self.full_basis_norb), dtype=np.bool_)
        ...
        self.mask_to_erme = np.zeros((len(self.bond_types), self.reduced_matrix_element), dtype=np.bool_)
        self.mask_to_nrme = np.zeros((len(self.type_names), self.reduced_matrix_element), dtype=np.bool_)
        ...
        if self.method == "e3tb":
            self.mask_to_ndiag = np.zeros((len(self.type_names), self.reduced_matrix_element), dtype=np.bool_)
            for ib, bb in self.basis.items():
                for io in bb:
                    iof = self.basis_to_full_basis[ib][io]
    ...
    def get_skonsite_maps(self):
        ...
        self.mask_diag = np.zeros(self.n_onsite_Es, dtype=np.bool_)
        self.skonsite_maps = {}
        for i, io in enumerate(self.full_basis):
