...
def _process_dict(kwargs, ignore_fields=[]):
    ...
    for k, v in kwargs.items():
        ...
        if k in _LONG_FIELDS:
            ...
            kwargs[k] = np.asarray(v, dtype=np.int64)
...
class AtomicData(Data):
    ...
    def __init__(
        ...
        if _validate:
            ...
            if (
                ...
                assert self.atomic_numbers.dtype in (np.int64, np.int32)
    ...
    def without_nodes(self, which_nodes):
        ...
        new_index = -np.ones((self.num_nodes,), dtype=np.int64)
        new_index[mask] = np.arange(n_keeping, dtype=np.int64)
...
def neighbor_list_and_relative_vec(
    ...
    mask = np.ones((len(first_idex),), dtype=np.bool_)
