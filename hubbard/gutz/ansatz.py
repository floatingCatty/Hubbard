...
from hubbard.data import OrbitalMapper
from hubbard.utils.constants import atomic_num_dict_r, atomic_num_dict
import copy
from hubbard.utils.tools import hermitian_basis_nspin, trans_basis_nspin
from hubbard.solver import ED_solver, NQS_solver, DMRG_solver
from hubbard.gutz.mixing import Linear, PDIIS
...
class gGASingleOrb(object):
    def __init__(
        ...
        if decouple_bath:
            assert self.solver == "DMRG" or self.solver == "NQS" or self.solver == "ED", "decouple bath for other method is not implemented"
        if natural_orbital:
            assert self.solver in ["NQS", "ED", "DMRG"]

        if nspin == 1:
            # Nparticle = [(self.norb*(self.naux+1)-i,i) for i in range(0,self.norb*(self.naux+1)+1)]
            Nparticle = [(norb*(naux+1)//2,norb*(naux+1)//2)]
        else:
            # self.Nparticle = [(self.norb*(self.naux+1)//2,self.norb*(self.naux+1)//2)]
            Nparticle = [(norb*(naux+1)-i,i) for i in range(0,norb*(naux+1)+1)]
    ...
    def update(self, t, intparam, E_fermi):
        ...
        self.solver.solve(_t, _intparam)
        RDM = self.solver.RDM()
    ...
    @property
    def E(self):
        return self.solver.E(True)
    
    @property
    def docc(self):
        return self.solver.docc(True)

    @property
    def S2(self):
        return self.solver.S2(True)
...
class gGAtomic(object):
    ...
    @property
    def S2(self):
        S2 = {sym:[[np.zeros(1) for _ in range(len(self.idp_phy.listnorbs[sym]))]]*int((self.atomic_number == atomic_num_dict[sym]).sum()) 
               for sym in self.idx_intorb.keys()}
